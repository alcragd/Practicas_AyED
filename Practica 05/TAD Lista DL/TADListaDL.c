/*
IMPLEMENTACIONES DE LA LIBRERIA DEL TAD LISTA (TADListaDL.h)
AUTOR ORIGINAL: Edgardo Adrián Franco Martínez (Diciembre 2022) (C)
IMPLEMENTA DOBLEMENTE LIGADA: Coyol Moreno Angel Zoe (Mayo 2025)
VERSIÓN: 1.8
...
La implementación del presente código se realiza mediante el principo de "Lista Doblemente Ligada"
i.e. nodos que contienen un elemento y se encuentran ligados hacia siguiente y anterior de estos.
...
posicion Previous(lista *l, posicion p)
{
	if (ValidatePosition(l, p))
		return p->anterior;
	printf("ERROR:Previous(L,p) la posicion es invalida");
	exit(1);
}
...
void VerLigasLista(lista *l)
{
	...
	while (aux != NULL)
	{
		printf("\nAnterior=%d\tPosicion=%d\tSiguiente=%d", aux->anterior, aux, aux->siguiente);
		aux = aux->siguiente;
	}
	printf("\n*************************************************************************");
